...
FRED_SERIES = [
    ...
    ('mfg',     'NAPM',            None),
    ('svc',     'NMFCI',           None),
    ('housing', 'HOUST',           None),   # in thousands — scaled below
    ('trade',   'BOPGSTB',         None),
    ('sent',    'UMCSENT',         None),
]
...
# ── Alpha Vantage fetch ───────────────────────────────
# Tries the actual index symbol; falls back to ETF and scales to index level.
AV_EQUITY = [
    # (key,     primary_symbol, fallback_etf, etf_scale_approx)
    ('sp500',  '^GSPC',        'SPY',         10.0),
    ('nasdaq', '^IXIC',        'QQQ',         None),   # QQQ tracks NDX not IXIC — skip scale
    ('dow',    '^DJI',         'DIA',         100.0),
]
...
def av_quote(symbol):
    ...
    if not price:
        raise ValueError(f'no quote for {symbol}: {list(gq.keys())}')
    return float(price), datetime.now(timezone.utc).strftime('%Y-%m-%d')

def av_fetch(key, primary, fallback, scale):
    """Try primary index symbol, fall back to ETF * scale."""
    # Try primary (actual index)
    try:
        val, date = av_quote(primary)
        return val, date
    except Exception as e:
        print(f'    AV {primary} failed ({e}), trying fallback ETF {fallback}')
...
for key, fred_id, units in FRED_SERIES:
    try:
        ...
        val = float(obs['value'])
        if key == 'housing':
            val = val / 1000          # thousands → millions
        results[key] = {
            'value':   round(val, 4),
            'date':    obs['date'],
        ...
        print(f'  ✓ FRED  {key:10s} {results[key]["display"]:>12s}  ({results[key]["label"]})')
    except Exception as e:
        errors.append(key)
        print(f'  ✗ FRED  {key:10s} ERROR: {e}')

# Fetch equities — Alpha Vantage primary, FRED fallback
for key, primary, fallback, scale in AV_EQUITY:
    ...
    if AV_KEY:
        try:
            ...
            print(f'  ✓ AV    {key:10s} {results[key]["display"]:>12s}  (today)')
            fetched = True
        except Exception as e:
            print(f'  ✗ AV    {key:10s} {e} — trying FRED fallback')
    ...
    if not fetched:
        ...
        if fred_id and FRED_KEY:
            try:
                ...
                print(f'  ✓ FRED  {key:10s} {results[key]["display"]:>12s}  (fallback, {results[key]["label"]})')
            except Exception as e:
                errors.append(key)
                print(f'  ✗ FRED  {key:10s} fallback also failed: {e}')
        else:
            errors.append(key)
...
print(f'\nWrote data.json — {len(results)}/18 fetched, {len(errors)} failed: {errors or "none"}')
